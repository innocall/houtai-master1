...
export default {
  ...
  computed: {
    ...
    // userXinxi(val,jiu){
      // if (typeof val.HRV == 'undefined' || val.HRV == '' || val.HRV == 'null' || !val.HRV) {
      //   return "暂无数据";
      // } else{
      //   return "有数据";
      // }
  ...
  created() {
    // this.cellValue = this.$route.query.cellValue;
    this.num = 1;
    this.type = "xy";
    this.loadType(this.type);
    this.loadData(this.userXinxi.Mobile);
    console.log(this.userXinxi,'777');
  },
  methods: {
    formatTimees(val) {
